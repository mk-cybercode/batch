...
export interface Expense {
  ...
  amount: number;
  supplierId?: ID;
  /** Older entries carry a flag meaning the whole amount came from the loan.
   *  `funding` supersedes it and is read first. */
  fundedFromCapital?: boolean;
  funding?: Funding;
...
/**
 * How an outlay was paid for. Amounts rather than a flag, because one purchase
 * is often split: a blast freezer might take R15,000 from the loan capital
 * with the rest put in personally. Whatever is left over after these two came
 * out of trading cash.
 */
export interface Funding {
  /** Drawn against the loan capital. */
  capital?: number;
  /** Put in from the owner's own pocket. */
  owner?: number;
}
